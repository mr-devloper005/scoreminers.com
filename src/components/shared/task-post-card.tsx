...
export function TaskPostCard({
  ...
  if (experience.key === "scoreminers") {
    if (variant === "image") {
      return (
        <Link href={href} className="group flex h-full flex-col overflow-hidden border-[3px] border-slate-950 bg-white shadow-[10px_10px_0_rgba(15,23,42,0.9)] transition hover:translate-x-[-2px] hover:translate-y-[-2px]">
          <div className="relative aspect-[4/5] border-b-[3px] border-slate-950 bg-[#fff5b4]">
            <ContentImage src={image} alt={altText} fill className="object-cover transition duration-500 group-hover:scale-[1.03]" />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950/80 via-slate-950/30 to-transparent p-4">
              <span className={`inline-flex px-3 py-1 text-[11px] font-black uppercase tracking-[0.2em] ${experience.badgeClass}`}>{category}</span>
            </div>
          </div>
          <div className="grid flex-1 gap-0">
            <div className="border-b-[3px] border-slate-950 p-5">
              <h3 className="text-2xl font-black uppercase leading-tight text-slate-950">{post.title}</h3>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 bg-white px-5 py-4">
              ...
              <span className="text-xs font-black uppercase tracking-[0.2em] text-slate-950">Open image</span>
            </div>
          </div>
        </Link>
      );
    ...
    if (variant === "profile") {
      return (
        <Link href={href} className="group grid h-full overflow-hidden border-[3px] border-slate-950 bg-white shadow-[10px_10px_0_rgba(15,23,42,0.9)] transition hover:translate-x-[-2px] hover:translate-y-[-2px] md:grid-cols-[0.9fr_1.1fr]">
          <div className="relative min-h-[260px] border-b-[3px] border-slate-950 bg-[#fff5b4] md:border-b-0 md:border-r-[3px]">
            <ContentImage src={image} alt={altText} fill className="object-cover transition duration-500 group-hover:scale-[1.03]" />
          </div>
          <div className="grid gap-0">
            <div className="border-b-[3px] border-slate-950 p-5">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-xs font-black uppercase tracking-[0.2em] text-slate-950">Profile</span>
              </div>
