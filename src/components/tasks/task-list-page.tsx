...
export async function TaskListPage({ task, category }: { task: TaskKey; category?: string }) {
  ...
  const ui = {
    ...
    linkStrong: 'font-semibold text-foreground underline-offset-4 hover:text-primary hover:underline',
  }
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {layoutKey === 'listing-directory' || layoutKey === 'listing-showcase' ? (
          <section className="mb-12 grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div className={`rounded-[2rem] p-7 shadow-[0_24px_70px_rgba(15,23,42,0.07)] ${ui.panel}`}>
              ...
              <h1 className={`mt-4 text-4xl font-semibold tracking-[-0.04em] ${ui.heading}`}>{taskConfig?.description || 'Latest posts'}</h1>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link href="/images" className={`inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm font-semibold ${ui.soft}`}>Open gallery</Link>
              </div>
            </div>
        ...
        {layoutKey === 'article-editorial' || layoutKey === 'article-journal' ? (
          <section className="mb-12 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
            <div>
              ...
              <h1 className={`mt-3 max-w-4xl text-5xl font-semibold tracking-[-0.05em] ${ui.heading}`}>{taskConfig?.description || 'Latest posts'}</h1>
        ...
        {layoutKey === 'image-masonry' || layoutKey === 'image-portfolio' ? (
          <section className="mb-12 grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            <div>
              ...
              <h1 className={`mt-5 text-5xl font-semibold tracking-[-0.05em] ${ui.heading}`}>{taskConfig?.description || 'Latest posts'}</h1>
        ...
        {layoutKey === 'profile-creator' || layoutKey === 'profile-business' ? (
          <section className={`mb-12 rounded-[2.2rem] p-8 shadow-[0_24px_70px_rgba(15,23,42,0.1)] ${ui.panel}`}>
            <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
              ...
              <div>
                <p className={`text-xs uppercase tracking-[0.3em] ${ui.muted}`}>{taskConfig?.label || task}</p>
                <h1 className={`mt-3 text-4xl font-semibold tracking-[-0.05em] ${ui.heading}`}>Profiles with stronger identity, trust, and reputation cues.</h1>
        ...
        {layoutKey === 'classified-bulletin' || layoutKey === 'classified-market' ? (
          <section className="mb-12 grid gap-4 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
            <div className={`rounded-[1.8rem] p-6 ${ui.panel}`}>
              <p className={`text-xs uppercase tracking-[0.3em] ${ui.muted}`}>{taskConfig?.label || task}</p>
              <h1 className={`mt-3 text-4xl font-semibold tracking-[-0.05em] ${ui.heading}`}>Fast-moving notices, offers, and responses in a compact board format.</h1>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {['Quick to scan', 'Shorter response path', 'Clearer urgency cues'].map((item) => (
                <div key={item} className={`rounded-[1.5rem] p-5 ${ui.soft}`}>
                  <p className={`text-sm font-semibold ${ui.heading}`}>{item}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        {layoutKey === 'sbm-curation' || layoutKey === 'sbm-library' ? (
          <section className="mb-12 grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
            <div>
              <p className={`text-xs uppercase tracking-[0.3em] ${ui.muted}`}>{taskConfig?.label || task}</p>
              <h1 className={`mt-3 text-4xl font-semibold tracking-[-0.05em] ${ui.heading}`}>Curated resources arranged more like collections than a generic post feed.</h1>
        ...
        {intro ? (
          <section className={`mb-12 rounded-[2rem] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)] sm:p-8 ${ui.panel}`}>
            <h2 className={`text-2xl font-semibold ${ui.heading}`}>{intro.title}</h2>
            {intro.paragraphs.map((paragraph) => (
              <p key={paragraph.slice(0, 40)} className={`mt-4 text-sm leading-7 ${ui.muted}`}>{paragraph}</p>
            ))}
            <div className="mt-4 flex flex-wrap gap-4 text-sm">
              {intro.links
                .filter((link) => link.href.startsWith('/images') || link.href.startsWith('/profile'))
                .map((link) => (
                  <a key={link.href} href={link.href} className={ui.linkStrong}>{link.label}</a>
                ))}
            </div>
          </section>
        ) : null}
